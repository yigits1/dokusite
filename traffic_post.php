...
if (isset($_POST['name']) && isset($_POST['phone']) && isset($_POST['tcno']) && isset($_POST['plate']) && isset($_POST['teklifTipi']) && isset($_POST['documentno'])) {
    ...
    if (empty($name)) {
        ...
        $insert_query = "INSERT INTO request(Name, Phone, TCNo, Plate, DocumentNo, RequestType, Type) VALUES (?, ?, ?, ?, ?, ?, ?)";
        $insert = $conn->prepare($insert_query);
        $insert->execute([$name, $phone, $tcno, $plate, $documentno, 'Trafik Sigortası', $teklifTipi]);
